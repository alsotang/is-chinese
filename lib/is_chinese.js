...
const commonAsciiCharsRe = /\w|\s|\t|\r|\n/

const rangeCheck = new Function('str', `
  for (let i = 0, charCode; i < str.length; charCode <= 0xffff ? i++ : i+=2) {
    charCode = str.codePointAt(i);
    ${
      chineseRange.map(range => {
        if (!Array.isArray(range)) {
          return `if (charCode === ${range}) {
            continue
          }`
        }
        return `if (charCode >= ${range[0]} && charCode <= ${range[1]}) {
          continue
        }`
      }).join('\n')
    }
